...
## [#48] · docs/readme-refresh-and-consistency-fixes · 2026-07-16 · docs: local star-history chart, README refresh, and doc-consistency fixes

The README's star-history chart stopped rendering because `api.star-history.com` returns `503 All GitHub API tokens are rate-limited` for every repo (reproduced against `facebook/react`, so it is service-wide, not specific to this repo or its markup), so it is replaced by `scripts/star_history.py` + a `make star-history` target that reads `starred_at` via `gh` and hand-renders committed light/dark SVGs into `assets/` — no new dependencies, and the `<picture>` block now points at files in this repo. Refreshed the README landing page (keyword-first H1, badges kept at top) and fixed a setup-breaking error in step 8, which named CI secrets `DATABRICKS_PRINCIPAL_ID`/`DATABRICKS_SECRET` while `onpush.yml` actually reads `DATABRICKS_CLIENT_ID`/`DATABRICKS_CLIENT_SECRET`/`TEMPLATE_ALERT_EMAILS` — anyone following it configured CI auth that silently failed. Corrected the specs against the code (documented `--load-test`, dropped the false `bundle validate` CI step and the `--env local` claim the parser rejects, noted `paths-ignore` and the `health_check`/integration-test retry exceptions, restored `job1_sdp/` to the folder tree, un-blanketed the `.claude/` gitignore claim, documented `make whoami`/`project-costs`) and de-duplicated CLAUDE.md, where the `resources/jobs.yml` rule had been stated four times; and widened the pre-commit ruff lint hook from `^src/` to `^(src|scripts)/`, since `scripts/` had never been linted — hiding 7 now-fixed errors (unsorted imports, trailing whitespace inside a SQL literal, an unused `requests` import) — with `known-first-party = ["_sdk_sql"]` added to `pyproject.toml` so isort stops filing that local sibling under the third-party `databricks` imports.
