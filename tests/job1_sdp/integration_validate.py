...
class ValidateSdp(BaseTask):
    def __init__(self, config):
        super().__init__(config)

    def run(self):
        self.logger.info("validating SDP integration tests")

        catalog = self.config.get_value("catalog")

        df_out = self.spark.table(f"{catalog}.report.order_agg_sdp")

        count = df_out.count()
        if count != 2:
            raise RuntimeError(f"Expected 2 rows in report.order_agg_sdp, got {count}")
