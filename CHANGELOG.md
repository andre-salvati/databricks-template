...
## [#29](https://github.com/andre-salvati/databricks-template/pull/29) · 2026-05-29 · feat: scale load-test data and rewrite seed_sources with initial load + daily incremental CDC

Scaled load-test seed volumes from 200 customers / 500k orders / 200 order_items to 500 customers / 2M orders / 6M order_items (3 items per order). The previous design seeded only 200 order_items against 500k orders so most orders joined to nothing and the compute was trivially fast — no meaningful gap between batch and incremental runs. Updated `_validate_load_test` assertions: 500 expected rows, `total_qty=24_000` and `total_value=600_000.0` per customer. Also rewrote `seed_sources` to use a two-phase design: on first run (empty tables) it performs a full initial load of 500 customers / 2M orders / 6M order_items via `spark.range()`, giving prod a realistic baseline for the batch vs incremental comparison; on every subsequent daily run it appends 2000 new orders (+1 item each, MERGE-idempotent by PK) and updates the `country` field for 50 customers (cycling through all 500 every 10 days), simulating realistic daily CDC activity that `job1_sdp` can detect and process incrementally. First-run detection is automatic via row-count check — no new CLI parameter.
